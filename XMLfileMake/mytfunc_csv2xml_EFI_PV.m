...
for iUNIT = 11:size(PVDataCell,1)
    ...
    % 名称と発電効率が空欄の場合はスキップ
    if isempty(PVDataCell{iUNIT,1}) == 0 && isempty(PVDataCell{iUNIT,4}) == 0
        ...
        % 太陽電池の種類
        if isempty(PVDataCell{iUNIT,2}) == 0
            if strcmp(PVDataCell(iUNIT,2),'結晶系')
                PV_Type = [PV_Type;'Crystalline'];
            elseif strcmp(PVDataCell(iUNIT,2),'結晶系以外')
        ...
        if isempty(PVDataCell{iUNIT,3}) == 0
            if strcmp(PVDataCell(iUNIT,3),'架台設置形')
                PV_InstallationMode = [PV_InstallationMode;'RackMountType'];
            elseif strcmp(PVDataCell(iUNIT,3),'屋根置き形')
                PV_InstallationMode = [PV_InstallationMode;'RoomMountType'];
            elseif strcmp(PVDataCell(iUNIT,3),'その他')
        ...
        if isempty(PVDataCell{iUNIT,4}) == 0
            PV_Capacity = [PV_Capacity; PVDataCell{iUNIT,4}];
        else
        ...
        if isempty(PVDataCell{iUNIT,5}) == 0
            PV_PanelDirection = [PV_PanelDirection; PVDataCell{iUNIT,5}];
        else
        ...
        if isempty(PVDataCell{iUNIT,6}) == 0
            PV_PanelAngle = [PV_PanelAngle; PVDataCell{iUNIT,6}];
        else
