...
ISR_V   = perDB_WEATHER(:,19);  % âîíº
ISR_H   = perDB_WEATHER(:,20);  % êÖïΩ
% ñÈä‘ï˙éÀ[Wh/m2]
NSR_V   = perDB_WEATHER(:,21);  % âîíº
NSR_H   = perDB_WEATHER(:,22);  % êÖïΩ


% % ã{ìáÇ≥ÇÒçÏê¨ÉtÉ@ÉCÉãÇì«Ç›çûÇﬁèÍçá
...
for iROOM = 1:numOfRoooms
    ...
    % äOîÁïââ◊ÇÃåvéZÅiï«ñáêîÇ™1à»è„Ç©Ç¬ÉfÅ[É^ÉxÅ[ÉXÇ…ê›íËÇ≥ÇÍÇΩäOï«Ç™Ç†ÇÈÇÃÇ∆Ç´Åj
                    ...
                    if strcmp(WallNameList{iDB},WallConfigure{iENV,iWALL})
                        ...
                        switch Direction{iENV,iWALL}
                            ...
                            case 'Horizontal'
                                ...
                                Qwall_S(:,iROOM) = Qwall_S(:,iROOM) + WallUA.*(0.8*0.04).*(DSR_H+ISR_H);  % ì˙éÀîMéÊìæ(365ì˙ï™)
                                Qwall_N(:,iROOM) = Qwall_N(:,iROOM) - WallUA.*(0.9*0.04).*NSR_H;          % ñÈä‘ï˙éÀ(365ì˙ï™)
                            ...
                            case {'Shade','Underground'}
                                ...
                                % ì˙éÀÇÕâΩÇ‡ë´Ç≥Ç»Ç¢ Å®Å@ì˙âAÇÃèÍçáÇÃÇ›ÅAñÈä‘ï˙éÀÇë´Ç∑ÅBÅièCê≥20130419Åj
                                if strcmp(Direction{iENV,iWALL},'Shade')
                                   Qwall_N(:,iROOM) = Qwall_N(:,iROOM) - WallUA.*(0.9*0.04).*NSR_V;  % ñÈä‘ï˙éÀ(365ì˙ï™)
                                end
                            ...
                            otherwise
                                ...
                                eval(['Qwall_S(:,iROOM) = Qwall_S(:,iROOM) + WallUA.*(0.8*0.04).*(DSR_',Direction{iENV,iWALL},'+ISR_V);']);  % ì˙éÀîMéÊìæ(365ì˙ï™)
                                Qwall_N(:,iROOM) = Qwall_N(:,iROOM) - WallUA.*(0.9*0.04).*NSR_V;  % ñÈä‘ï˙éÀ(365ì˙ï™)
                        end
                    end
                end
                    ...
                    if strcmp(WindowNameList{iDB},WindowType{iENV,iWALL})
                        ...
                        switch Direction{iENV,iWALL}
                            case 'Horizontal'
                                ...
                                for dd = 1:365
                                    if SeasonMode(dd) == -1  % ígñ[
                                        Qwind_S(dd,iROOM) = Qwind_S(dd,iROOM) + WindowEavesH.* (WindowSCC+WindowSCR).*(DSRita_H(dd)*0.89+ISR_H(dd)*0.808); % ì˙éÀîMéÊìæ(365ì˙ï™)
                                    else
                                        Qwind_S(dd,iROOM) = Qwind_S(dd,iROOM) + WindowEavesC.* (WindowSCC+WindowSCR).*(DSRita_H(dd)*0.89+ISR_H(dd)*0.808); % ì˙éÀîMéÊìæ(365ì˙ï™)
                                    end
                                end
                                
                                Qwind_N(:,iROOM) = Qwind_N(:,iROOM) - WindowUA.*(0.9*0.04).*NSR_H;  % ñÈä‘ï˙éÀ(365ì˙ï™)
                                
                            case {'Shade','Underground'}
                                
                                % ì˙âAÇÃèÍçáÇÃÇ›ÅAñÈä‘ï˙éÀÇë´Ç∑ÅBÅièCê≥20130419Åj
                                if strcmp(Direction{iENV,iWALL},'Shade')
                                    Qwind_N(:,iROOM) = Qwind_N(:,iROOM) - WindowUA.*(0.9*0.04).*NSR_V;  % ñÈä‘ï˙éÀ(365ì˙ï™)
                                end
                            ...
                            otherwise
                                ...
                                for dd = 1:365
                                    if SeasonMode(dd) == -1  % ígñ[
                                        eval(['Qwind_S(dd,iROOM) = Qwind_S(dd,iROOM) + WindowEavesH.*(WindowSCC+WindowSCR).*(DSRita_',Direction{iENV,iWALL},'(dd)*0.89+ISR_V(dd)*0.808);']) % ì˙éÀîMéÊìæ(365ì˙ï™)
                                    else
                                        eval(['Qwind_S(dd,iROOM) = Qwind_S(dd,iROOM) + WindowEavesC.*(WindowSCC+WindowSCR).*(DSRita_',Direction{iENV,iWALL},'(dd)*0.89+ISR_V(dd)*0.808);']) % ì˙éÀîMéÊìæ(365ì˙ï™)
                                    end
                                end
