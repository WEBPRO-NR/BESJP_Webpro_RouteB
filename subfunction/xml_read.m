...
DPref.Debug     = false; % show specific errors (true) or general (false)?
tree     = [];
RootName = [];
...
if (nargin>1)
  ...
  if (isfield(Pref, 'Debug'   )), DPref.Debug    = Pref.Debug   ; end
end
...
while (~isempty(Node))
  ...
  elseif (Node.getNodeType==Node.PROCESSING_INSTRUCTION_NODE)
    ...
    GlobalTextNodes{2} = [target, ' ', data]; 
  elseif (Node.getNodeType==Node.COMMENT_NODE)
    GlobalTextNodes{3} = strtrim(char(Node.getData));
  end
...
%% If there were any Global Text nodes than return them return them
