...
for iRESION = 1:8
    ...
    if iRESION == 1 || iRESION == 2
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_病院_Ia_Ib.xlsx','計算ケース一括');
    elseif iRESION == 3 || iRESION == 4
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_病院_II_III.xlsx','計算ケース一括');
    elseif iRESION == 5 || iRESION == 6 || iRESION == 7
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_病院_IVa_IVb_V.xlsx','計算ケース一括');
    elseif iRESION == 8
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_病院_VI.xlsx','計算ケース一括');
    else
        error('リージョンコードが不明です')
    end
        ...
        for iBLDG = 1:3
            ...
            for iMODEL = 1:3
                ...
                for iDIRECTION = 1:4
                    ...
                    checkroomtype = 1;
                    if strcmp(caseSet{2}(tmpbar(end)+1:end),'NaN')
                        checkroomtype = 0;
                    end
                    
                    if strcmp(caseSet{2}(tmpbar(end)+1:end),'病室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '病室';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'診察室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '診察室';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'待合室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '待合室';
                        Fh = 4.25; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'検査部')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '検査部';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'病理部')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '病理部';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'事務室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '事務室';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'NS')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'NS';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'ICU')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'ICU';
                        Fh = 5.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'食堂')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '食堂';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'手術室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '手術室';
                        Fh = 5.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'住戸')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '住戸';
                        Fh = 3.1; %  階高 [m]
                    else
                        if strcmp(caseSet{2}(tmpbar(end-1)+1:end),'廊下_病室')
                            OUTPUT.Rooms.Room.ATTRIBUTE.Type = '廊下（病室）';
                            Fh = 4.0; %  階高 [m]
                        elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'廊下_中央')
                            OUTPUT.Rooms.Room.ATTRIBUTE.Type = '廊下（中央）';
                            Fh = 4.0; %  階高 [m]
                        else
                            error('該当する室用途がありません！')
                        end
