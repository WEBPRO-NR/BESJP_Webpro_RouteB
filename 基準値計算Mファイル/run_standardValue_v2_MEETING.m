...
for iRESION = 1:8
    ...
    if iRESION == 1 || iRESION == 2
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_集会所_Ia_Ib.xlsx','計算ケース一括');
    elseif iRESION == 3 || iRESION == 4
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_集会所_II_III.xlsx','計算ケース一括');
    elseif iRESION == 5 || iRESION == 6 || iRESION == 7
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_集会所_IVa_IVb_V.xlsx','計算ケース一括');
    elseif iRESION == 8
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_集会所_VI.xlsx','計算ケース一括');
    else
        error('リージョンコードが不明です')
    end
        ...
        for iBLDG = 1:3
            ...
            for iMODEL = 1:3
                ...
                for iDIRECTION = 1:4
                    ...
                    if strcmp(caseSet{2}(tmpbar(end)+1:end),'パチンコ')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'パチンコ';
                        Fh = 4.3; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'カラオケ')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'カラオケ';
                        Fh = 3.9; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'ネカフェ')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'ネカフェ';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'社寺本殿')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '社寺本殿';
                        Fh = 9.00; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'劇場等')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '劇場等';
                        Fh = 16.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'楽屋等')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '楽屋等';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'映画館')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '映画館';
                        Fh = 12.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'劇場ロビー')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '劇場ロビー';
                        Fh = 8.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'映画ロビー')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '映画ロビー';
                        Fh = 8.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'アスレ等')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'アスレ等';
                        Fh = 4.75; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'ボーリング')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'ボーリング';
                        Fh = 5.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'体育館')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '体育館';
                        Fh = 13.5; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'屋内プール')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '屋内プール';
                        Fh = 7.1; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'応援席等')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '応援席等';
                        Fh = 4.25; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'脱衣室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '脱衣室';
                        Fh = 3.9; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'競馬競輪場')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '競馬競輪場';
                        Fh = 9.1; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'アスレロビー')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'アスレロビー';
                        Fh = 4.8; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'体育ロビー')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '体育ロビー';
                        Fh = 4.8; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'競馬ロビー')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '競馬ロビー';
                        Fh = 4.8; %  階高 [m]
                    else
                        error('ERROR!!!')
                    end
