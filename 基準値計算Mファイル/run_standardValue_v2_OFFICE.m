...
for iRESION = [1,8]
    ...
    if iRESION == 1 || iRESION == 2
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_事務所_Ia_Ib.xlsx','計算ケース一括');
    elseif iRESION == 3 || iRESION == 4
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_事務所_II_III.xlsx','計算ケース一括');
    elseif iRESION == 5 || iRESION == 6 || iRESION == 7
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_事務所_IVa_IVb_V.xlsx','計算ケース一括');
    elseif iRESION == 8
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v8入力_事務所_VI.xlsx','計算ケース一括');
    else
        error('リージョンコードが不明です')
    end
        ...
        for iBLDG = 1:3
            ...
            for iMODEL = 1:3
                ...
                for iDIRECTION = 1:4
                    ...
                    if strcmp(caseSet{2}(tmpbar(end)+1:end),'事務室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '事務室';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'会議室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '会議室';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'ロビー')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'ロビー';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'図書館')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '図書館';
                        Fh = 6.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'博物館')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '博物館';
                        Fh = 6.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'電算室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '電算室';
                        Fh = 4.8; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'廊下')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '廊下';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'中央監視室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '中央監視室';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'更衣室等')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '更衣室等';
                        Fh = 4.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'社食')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '社食';
                        Fh = 4.0; %  階高 [m]
                    else
                        if strcmp(caseSet{2}(tmpbar(end-1)+1:end),'事務室_高')
                            OUTPUT.Rooms.Room.ATTRIBUTE.Type = '事務室（高）';
                            Fh = 4.0; %  階高 [m]
                        else
                            error('該当する室用途がありません！')
                        end
