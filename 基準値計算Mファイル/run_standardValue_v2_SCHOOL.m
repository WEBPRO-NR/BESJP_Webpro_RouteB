...
for iRESION = 1:8
    ...
    if iRESION == 1 || iRESION == 2
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_学校_Ia_Ib.xlsx','計算ケース一括');
    elseif iRESION == 3 || iRESION == 4
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_学校_II_III.xlsx','計算ケース一括');
    elseif iRESION == 5 || iRESION == 6 || iRESION == 7
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_学校_IVa_IVb_V.xlsx','計算ケース一括');
    elseif iRESION == 8
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v6入力_学校_VI.xlsx','計算ケース一括');
    else
        error('リージョンコードが不明です')
    end
        ...
        for iBLDG = 1:3
            ...
            for iMODEL = 1:3
                ...
                for iDIRECTION = 1:4
                    ...
                    if strcmp(caseSet{2}(tmpbar(end-1)+1:end),'幼稚園_小中高教室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '小中高教室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'幼稚園_事務室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '事務室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'幼稚園_職員室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '職員室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'小学校・中学校_小中高教室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '小中高教室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'小学校・中学校_事務室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '事務室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'小学校・中学校_講堂')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '講堂';
                        Fh = 9.1; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'小学校・中学校_パソコン室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'パソコン室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'小学校・中学校_職員室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '職員室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'高等学校_事務室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '事務室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'高等学校_小中高教室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '小中高教室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'高等学校_講堂')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '講堂';
                        Fh = 9.1; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'高等学校_パソコン室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'パソコン室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'高等学校_職員室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '職員室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'大学_大学教室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '大学教室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'大学_事務室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '事務室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'大学_講堂')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '講堂';
                        Fh = 9.1; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'大学_研究室等')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '研究室等';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'大学_パソコン室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'パソコン室';
                        Fh = 4.2; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end-1)+1:end),'大学_職員室')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '職員室';
                        Fh = 4.2; %  階高 [m]
                        
                    else
                        
                        if strcmp(caseSet{2}(tmpbar(end-2)+1:end),'幼稚園_食堂_幼小中高')
                            OUTPUT.Rooms.Room.ATTRIBUTE.Type = '食堂（幼小中高）';
                            Fh = 4.2; %  階高 [m]
                        elseif strcmp(caseSet{2}(tmpbar(end-2)+1:end),'小学校・中学校_食堂_幼小中高')
                            OUTPUT.Rooms.Room.ATTRIBUTE.Type = '食堂（幼小中高）';
                            Fh = 4.2; %  階高 [m]
                        elseif strcmp(caseSet{2}(tmpbar(end-2)+1:end),'高等学校_食堂_幼小中高')
                            OUTPUT.Rooms.Room.ATTRIBUTE.Type = '食堂（幼小中高）';
                            Fh = 4.2; %  階高 [m]
                        elseif strcmp(caseSet{2}(tmpbar(end-2)+1:end),'大学_食堂_大学')
                            OUTPUT.Rooms.Room.ATTRIBUTE.Type = '食堂（大学）';
                            Fh = 4.2; %  階高 [m]
                        else
                            error('該当する室用途がありません！')
                        end
