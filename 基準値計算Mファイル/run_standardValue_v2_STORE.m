...
for iRESION = 4:8
    ...
    if iRESION == 1 || iRESION == 2
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v9入力_物販_Ia_Ib.xlsx','計算ケース一括');
    elseif iRESION == 3 || iRESION == 4
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v9入力_物販_II_III.xlsx','計算ケース一括');
    elseif iRESION == 5 || iRESION == 6 || iRESION == 7
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v9入力_物販_IVa_IVb_V.xlsx','計算ケース一括');
    elseif iRESION == 8
        [NN,TT,RR] = xlsread('./標準仕様/基準値計算_建物モデル設定 v9入力_物販_VI.xlsx','計算ケース一括');
    else
        error('リージョンコードが不明です')
    end
        ...
        for iBLDG = 1:3
            ...
            for iMODEL = 1:3
                ...
                for iDIRECTION = 1:4
                    ...
                    checkroomtype = 1;
                    if strcmp(caseSet{2}(tmpbar(end)+1:end),'店舗')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = '店舗';
                        Fh = 5.0; %  階高 [m]
                    ...
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'ロビー')
                        ...
                        Fh = 5.0; %  階高 [m]
                    elseif strcmp(caseSet{2}(tmpbar(end)+1:end),'バック')
                        OUTPUT.Rooms.Room.ATTRIBUTE.Type = 'バック';
                        Fh = 4.0; %  階高 [m]
                    else
                        error('該当する室用途がありません！')
                    end
